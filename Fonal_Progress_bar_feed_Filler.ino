...
/* // test _ loop
void loop() {
  // 🛑 Always check for Emergency Stop
  ...
  if (running) {
    unsigned long elapsed = (currentMillis - runStartTime) / 1000;
    ...
    if (runStep == 0) {
      if (!motor1PulseDone) {
        ...
        digitalWrite(MOTOR1_2_PIN, LOW);
        delay(500);
        motor1PulseDone = true;
      }
      if ((currentMillis - runStartTime) < motor1Time * 60000UL) {
        int remaining = (motor1Time * 60) - elapsed;
        ...
        runStep = 1;
        runStartTime = millis();
        motor1PulseDone = false;  // Reset for Motor 2
      }

    // Gap delay before Motor 2
    } else if (runStep == 1) {
      if ((currentMillis - runStartTime) < gapTime * 60000UL) {
        int remaining = (gapTime * 60) - elapsed;
        updateRunLCD("Gap Interval", remaining);
      } else {
        runStep = 2;
        runStartTime = millis();
      }
    ...
    // Motor 2 logic
      ...
      if ((currentMillis - runStartTime) < motor2Time * 60000UL) {
        int remaining = (motor2Time * 60) - elapsed;
...
void loop() {
  ...
  if (running) {
    ...
    // --- Motor 2 Control ---
// --- Motor 2 Control ---
if (m1Started && !m1Ended && !m2Started) {
  if ((now - m1StartTime) >= gapTime * 60000UL) {
    // Start pulse
    if (!m2PulseStartDone) {
...
void handleInput(char key) {
  ...
  if (key >= '0' && key <= '9') {
    ...
    if (step == 0 && val >= 1 && val <= 60) { motor1Time = val; valid = true; }
    else if (step == 1 && val >= 1 && val <= 60) { motor2Time = val; valid = true; }
    else if (step == 2 && val >= 1 && val <= 5) { gapTime = val; valid = true; }
    else if (step == 3 && val >= 1 && val <= 5) { cycleCount = val; valid = true; }
...
void printProgressBar(int col, int row, int remaining, int total) {
  ...
  int bars = map(total - remaining, 0, total, 0, 10);
  for (int i = 0; i < 10; i++) {
    if (i < bars) lcd.print("#");
    else lcd.print("-");
  }
